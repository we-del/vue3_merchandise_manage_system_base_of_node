...
import basic_vertex from './shader/complex/vertex.glsl'
import basic_fragColor from './shader/complex/fragColor.glsl'
...
class ViewScene {
  ...
  public useShader(): void {
    ...
    // 加载纹理图片
    const textureLoader = new TextureLoader(); // 得到纹理加载器
    const textureShader = textureLoader.load(TextureShader) // 加载纹理图片
    /**
    ...
    console.log('@geometry', boxRealGeometry) // 顶点着色器可以获得当前几何体上的属性
    const boxRealEntity = new Mesh(boxRealGeometry, this.shaderMaterial)
    console.log('@entity', boxRealEntity) // 顶点着色器可以获得当前几何体上的属性
