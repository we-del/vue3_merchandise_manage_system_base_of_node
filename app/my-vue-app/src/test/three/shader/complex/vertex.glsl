...
/**
  @description 顶点着色器获得绘制点得相关变量
  @param  position 拿到当前几何体绘制点得 x,y,z轴
  @param  uv 拿到当前几何体绘制点得 x,y轴
*/

/**
  @description 着色器绘制属性(用于确定当前点位绘制得最终行为)
  @param gl_Position 绘制当前顶点得位置
  @param gl_FragColor 绘制当前片元得颜色
  @param gl_FrontFacing 判断当前绘制得点位是否为外侧展示点位(是则为true，不是则为false)
*/
void main() {
    // 得到几何体得模型
